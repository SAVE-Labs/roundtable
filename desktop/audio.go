...
func NewWebRTCClient(wsURL string, onRemotePCM16LE func([]byte)) (*WebRTCClient, error) {
	...
	serverOrigin := "http://127.0.0.1:1323"
	...
	// Handle server-initiated renegotiation offers
	go func() {
		for {
			var msgBytes []byte
			...
			reAnswer, err := peer.CreateAnswer(nil)
			if err != nil {
				continue
			}
			gatherDone := webrtc.GatheringCompletePromise(peer)
			if err := peer.SetLocalDescription(reAnswer); err != nil {
				continue
			}
			<-gatherDone
			localDesc := peer.LocalDescription()
			...
			ansBytes, err := json.Marshal(localDesc)
			if err != nil {
				continue
			}
			_ = websocket.Message.Send(ws, ansBytes)
		}
	}()
